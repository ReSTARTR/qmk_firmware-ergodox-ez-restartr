...
#define MODS_T(kc, mod) ACTION_MODS_TAP_KEY(kc, mod)
...
[BASE] = KEYMAP(  // layer 0 : default
        ...
        KC_GRV,         LGUI(KC_C),LGUI(KC_V),LGUI(KC_X),KC_LCTRL,
        ...
        // right hand
             ...
             C_S(KC_SCLN),     KC_N,   KC_M,   KC_COMM,KC_DOT, LT(SYMB, KC_SLSH),KC_COLN,
...
[MDIA] = KEYMAP(
       ...
       C(KC_RBRC), KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
       M(ALFRED_DASH),   KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,
                                           KC_TRNS, KC_TRNS,
                                                    KC_TRNS,
                                  KC_BTN1, KC_WBAK, KC_TRNS,
    // right hand
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  // MACRODOWN only works in this function
      switch(id) {
        ...
        break;
        case ALFRED_DASH:
        return MACRODOWN( D(LCTL), D(SPC), U(SPC), U(LCTL), W(50), T(D), W(40), T(A), W(30), T(S), W(20), T(H), W(10), T(SPC), END );
        case GUI_C:
        return MACRODOWN( D(LGUI), W(10), T(C), W(10), U(LGUI), END );
        case GUI_V:
        return MACRODOWN( D(LGUI), W(10), T(V), W(10), U(LGUI), END );
        case GUI_X:
        return MACRODOWN( D(LGUI), W(10), T(S), W(10), U(LGUI), END );
        case KONAMI:
    ...
    for (uint8_t row = 0; row < MATRIX_ROWS; row++) {
      for (uint8_t col = 0; col < MATRIX_COLS; col++) {
        if (matrix_is_on(row, col)) {
          ...
          action_t action = layer_switch_get_action(
